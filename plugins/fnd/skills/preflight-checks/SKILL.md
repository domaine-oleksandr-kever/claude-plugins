...
allowed-tools: Read, Glob, Bash(shopify version), Bash(node -v), Bash(npm -v), Bash(git --version), Bash(gh --version), Bash(jq --version), Bash(perl -v), Bash(${CLAUDE_PLUGIN_ROOT}/scripts/jira-attachments.sh --check*), Bash(git ls-remote*), Bash(timeout 8 git ls-remote*)
---
