...
every `<plugin root>/…` path below resolves the same way; on Claude Code, write plugin root as
the literal `${CLAUDE_PLUGIN_ROOT}` in every command (the host expands it, and this skill's
pre-approved Bash prefixes match that exact form).
