...
**Plugin root** = the plugin's own directory — this agent is `<plugin root>/agents-opencode/jira-reader.md` (the installer symlinks it into the OpenCode config dir — resolve the link to reach the bundle), and every `<plugin root>/…` path below resolves the same way; substitute its absolute path when a command has to run.
...
`<plugin root>/references/jira-freshness-check.md` and follow it — it can
...
`<plugin root>/references/jira-field-ids.md` (tiny — the verified custom-field
...
`<plugin root>/references/jira-custom-fields.md` → Step B, rediscover, use the
