...
**Plugin root** = the plugin's own directory — this agent is `<plugin root>/agents-opencode/figma-reader.md` (the installer symlinks it into the OpenCode config dir — resolve the link to reach the bundle), and every `<plugin root>/…` path below resolves the same way; substitute its absolute path when a command has to run.
...
3. **Per-element measurements — `get_design_context`, processed in FULL.** This holds the exact
   ...
   run `node <plugin root>/scripts/json-slim.cjs <file> --stats`** — `--stats` puts
