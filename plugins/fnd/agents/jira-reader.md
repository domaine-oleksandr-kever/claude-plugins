---
name: jira-reader
description: Reads ONE Jira ticket via the Atlassian MCP — fields, comments and attachments (images downloaded into the task workspace, screen recordings cut into frames there) — and returns them compactly, keeping the raw ADF and the bytes out of the main context. Use PROACTIVELY whenever a whole ticket needs reading — e.g. when a Jira URL or key (ABC-123) is pasted. One per ticket, in parallel; skip tickets already in context. Writes `ticket.md` and `comments.md` itself when given the workspace path. NOT for single-field lookups or JQL searches — use the MCP directly. Read-only toward Jira.
model: sonnet
effort: medium
...
recording is not kept) and the frames dir per video
...
all of them by default) — plus the `attachments_note` line when it is set. No attachments → the section says
so in one line.
