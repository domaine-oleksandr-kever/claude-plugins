...
3. **Per-element measurements — `get_design_context`, processed in FULL.** This holds the exact
   ...
   run `node ${CLAUDE_PLUGIN_ROOT}/scripts/json-slim.cjs <file> --stats`** — `--stats` puts
   `json-slim: <in> → <out> bytes (<pct>% reduction)` on stderr; report that line as
   `compression`, **verbatim**. A Figma design context gets
...
2. **Compact.**
   ...
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/figma-node-slim.cjs <the nodes.json> \
     --variables <the variables.json> --out <the nodes.json>.md --stats
   ```

   `--stats` adds `figma-node-slim: <in> B → <out> B (-NN.N%) nodes=N hidden=N folded=N` on
   stderr — carry that line into `compression` **verbatim**, counters and all.
...
that answered — `mcp-connector` / `mcp-desktop` / `rest`), `compression` (the same line you
return — a `/compact` drops the transcript, not the file), `last_modified` (rung 3 only: the
...
compression:                # what compression this read actually got — the scripts' own stderr lines, VERBATIM, `; `-joined, never re-rounded or re-worded, e.g. "json-slim: 196608 → 45012 bytes (77.1% reduction); figma-node-slim: 196608 B → 30104 B (-84.7%) nodes=412 hidden=63 folded=180"; "none" when neither ran
needs_clarification:        # "" if none; else a one-line question for the developer
saved_to:                   # workspace file path, or "" if not saved
```

`source_url`, `source`, `frame`, `compression`, `needs_clarification` and `saved_to` always come back. `spec` and
`assets`
