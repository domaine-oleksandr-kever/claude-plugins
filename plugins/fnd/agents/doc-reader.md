...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/json-slim.cjs <path> --stats` and work from its stdout;
`--stats` prints `json-slim: <in> → <out> bytes (<pct>% reduction)` on stderr — report that line
as `compression`, **verbatim**
(`--jq` narrows to a sub-tree — dot paths, `[]` iteration, `,` multi-select and `| keys` /
`| length` only, anything else exits 2; a wrong path yields `null`, not an error — and it takes
no `--stats`: a sub-path is a narrowing, not a compression).
...
MCP returns one), `compression` (the same line you return — a `/compact` drops the transcript,
not the file), `provenance: untrusted` (the extract is fetched content — readers of the
