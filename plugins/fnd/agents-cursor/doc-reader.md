...
**Plugin root** = the plugin's own directory — this agent is `<plugin root>/agents-cursor/doc-reader.md`, and every `<plugin root>/…` path below resolves the same way; substitute its absolute path when a command has to run.
