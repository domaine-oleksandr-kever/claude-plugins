...
# model: pinned per references/host-model-map.md — the `standard` tier pin, overridden per-agent
#        (Domaine guidelines). Ids verified 2026-08-23.
# override: scoped task recon sits at the guidelines' Composer bar, not the tier's Sonnet bar — the tier still governs its Codex pin
# escalate: claude-sonnet-5 (next rung of the ladder — the pin is where it starts)
name: theme-explorer
...
**Plugin root** = the plugin's own directory — this agent is `<plugin root>/agents-cursor/theme-explorer.md`, and every `<plugin root>/…` path below resolves the same way; substitute its absolute path when a command has to run.
