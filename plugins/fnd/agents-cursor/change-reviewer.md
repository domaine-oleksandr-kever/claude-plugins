...
# model: pinned per references/host-model-map.md — the `standard` tier pin
#        (Domaine guidelines). Ids verified 2026-08-23.
# escalate: claude-opus-5 (next rung of the ladder — the pin is where it starts)
...
**Plugin root** = the plugin's own directory — this agent is `<plugin root>/agents-cursor/change-reviewer.md`, and every `<plugin root>/…` path below resolves the same way; substitute its absolute path when a command has to run.
...
**No `profile` in the brief?** Run `bash "<plugin root>/scripts/project-profile.sh"`
