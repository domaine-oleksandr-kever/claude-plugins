...
| Subagent not spawnable | the host's agent directory for this install is missing or not discovered — reinstall, then start a new session. On Codex the roles load from `~/.codex/agents` only: run `scripts/install.sh --target codex`, then `doctor.cjs --target codex` — its `codex:agents` row says whether the links are live |
