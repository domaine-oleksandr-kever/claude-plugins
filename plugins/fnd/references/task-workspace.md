...
| `tmp/attachments/` | the ticket's downloaded images, `<attachment-id>-<sanitised-name>`; a **video is not kept** — it leaves one `<file>.frames/` dir of PNGs (timecoded, `05-00m20s.png`); `Read` cannot open a dir, so the caller lists it and `Read`s the `<NN>-<MM>m<SS>s.png` frames the task needs, never all of them by default; a screenshot the ticket **links** (prnt.sc, imgur, Gyazo, CleanShot, snipboard) lands beside them as `<host>-<slug>.<ext>`, downscaled to ≤1600 px wide | `jira-reader` (via `scripts/jira-attachments.sh` and `scripts/external-screenshots.sh`) |
