...
// stub naming the spill path and the json-slim command to run on it, instead of landing in context
...
const { slim, sweepSpills, spillRoot, debugLog, debugEnabled, shapeHint } = require('../scripts/json-slim.cjs');
...
// contract: run the json-slim CLI on the file, never raw-Read a whale. The shape hint is the only
// DROPPABLE part — it goes whole if the cap is reached, leaving a few fixed lines plus the two paths
// (tool name capped at STUB_TOOL_MAX), comfortably under the cap. The command line must survive intact.
function stubText(tool, bytes, format, hint, file) {
...
function buildStub(result, tool, format, stubLimit) {
  const p = payloadOf(result); // shape probe BEFORE any file is written
  if (p === null || p.bytes <= stubLimit) return null;
  const file = spillOriginal(p.payload);
  if (!file) return null; // no recovery copy → RAW passthrough (never lose the only copy)
  const h = shapeHint(p.payload);
  const fmt = format || h.format;
  return { value: stubValue(result, stubText(tool, p.bytes, fmt, h.hint, file)), spill: file, format: fmt };
}
...
function run(raw) {
  ...
  const tryStub = (reason, stages) => {
    const s = buildStub(result, tool, slimmed.format, stubLimit);
    if (!s) return false;
    emit(s.value);
