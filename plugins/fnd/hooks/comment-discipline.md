...
function/snippet interface docs; schema/config docs. An interface doc covers the contract
of THAT field/param only (units, invariants, sentinel values) — why *other* code does or
doesn't do something is not its business. An architectural WHY / trade-off note: 1–3 lines
max; anything longer belongs in the commit/PR body (or the task workspace `notes.md`),
never in code. Skip doc that merely restates the signature.
