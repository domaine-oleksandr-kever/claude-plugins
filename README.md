...
- **UserPromptSubmit** — one node process (`user-prompt.cjs`) running three independently
  ...
  - **PostToolUse (`mcp__.*`)** — `mcp-slim.cjs` compresses large MCP tool results before they
    ...
    line and the handle included. A passthrough says nothing (nothing changed), and this is not a
    ...
    `/compact`. A reader runs in its own context, so the skill that spawned it relays that field to
    the developer in one line once the ingest reads are done
    (`references/task-workspace.md` → Read rule, compression) — the transcript is the only place
    they see it. `figma-node-slim --stats` does the same for a Figma REST tree. `--stats` is left OFF
