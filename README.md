...
- **PreToolUse (the two browser screenshot tools) — scratch-path guard.**
  ...
  output path resolves inside the project working tree but outside `.claude/` — the untracked
  QA litter `references/task-workspace.md` already forbids in prose — with a reason that hands
  the model the path to use instead (`.claude/tasks/<work-id>/tmp/<name>`, or
  `.claude/tmp/<name>` with no ticket; both stay inside the workspace, which the playwright
  server requires of any file it writes, and the guard **creates** them as it denies, since
  nothing else does and the retry would otherwise fail with ENOENT). A playwright call with
  **no** `filename` is denied too — that one does not skip the write, it falls back to
  `<cwd>/.playwright-mcp/`, inside the checkout — while chrome-devtools without `filePath`
  returns the image inline and passes. Everything else passes as well: anything under
  `.claude/`, an absolute path outside the checkout. An in-project `tmp/` does **not** pass:
  a theme checkout neither ships nor gitignores one, so it is just the next place the litter
  lands.
