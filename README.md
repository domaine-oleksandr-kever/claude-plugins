...
| `CLAUDE_CODE_SESSION_ID` | set by Claude Code | *read, not set by fnd*: scopes `FND_WHALE_GUIDE`'s one-shot state to the conversation, so a new session sees the full guidance block again. Absent (a bare shell) ⇒ the state is keyed on the file path alone and the 2 h expiry bounds it |
