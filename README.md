...
| MCP result handling | **observe-only** — `afterMCPExecution` exposes no rewrite field, so nothing is compressed, stubbed or spilled (the shim logs `skip`) | no output rewrite — spill-and-stub only, via `additionalContext` | in-place rewrite (compress **and** spill-and-stub) |
...
- **UserPromptSubmit** — one node process (`user-prompt.cjs`) running two independently
  ...
  - **PostToolUse (`mcp__.*`)** — `mcp-slim.cjs` compresses large MCP tool results before they
    ...
    so nothing is lost. Stale spills are swept by an mtime TTL (`FND_MCP_SLIM_TTL`) — and the bundled
...
| `FND_MCP_SLIM_DEBUG` | off | opt-in: append one JSONL trace line per `mcp-slim` / `json-slim` invocation to `<FND_MCP_SLIM_DIR>/fnd-mcp-slim-debug.log` (project, `lvl`, decision, reason, `format` on non-json, `narrowed` on a `--jq` CLI run, `guide` (`full`/`reminder`) on a JSONL profile, `budget_partial` on a mid-array wall-clock expiry, `delivery`/`delivered` + `host` on a host that cannot replace a tool result (see below), bytes, %, stages, `spills` — never any payload); rotates one generation at ~5 MB. `1` (or `true`/`yes`/`on`) = **key events**: everything except the sub-gate `size-gate` lines for results under the 4 KB gate, which were ~76 % of a real week's log. `2` (any integer ≥ 2) = **everything**, sub-gate lines included. Unset / `0` / `false` / an unrecognized value ⇒ no file written. The level rides on each line as `lvl`, so `--report` can say which of its numbers cover a partial event set — a `1` window's totals % is not comparable with a `2` window's. **Host divergence:** where the host adapter cannot replace a tool result (Codex), the shrunk lines also carry what was DELIVERED — `delivery` (`discard` = the compressed body was dropped, `additional` = a stub rode back beside the raw result) plus `delivered`, the bytes of that stub — and `--report` counts those as 0 saved (and, for `additional`, as context ADDED), with a `delivery:` line naming the split |
| `FND_MCP_SLIM_STUB` | `1` | `0` disables the spill-and-stub guard: a result the compressor cannot bring under the threshold is then handed to the model raw again, instead of as a ~1 KB stub + spill. **Host divergence:** on Codex the stub is the only half of `mcp-slim` that survives (no output rewrite there), forwarded as `additionalContext` — so `0` on that host means a whale lands in context with nothing said about the spill that holds it. On Cursor there is no stub at all: `afterMCPExecution` is observe-only, so neither half of `mcp-slim` lands and this switch changes nothing there |
