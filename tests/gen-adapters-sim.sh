...
fingerprint() {
  ...
  ( cd "$1" 2>/dev/null || return 0
    { find $GEN_DIRS opencode -type f 2>/dev/null
      ...
      for f in rules/fnd-*.mdc; do [ -f "$f" ] && echo "$f"; done
...
# -------------------------------------------- the Cursor rules generated from the session hooks --
...
for n in $HOOK_RULES; do
  ...
  else bad "rule-genmark-$n" "fnd-$n.mdc does not mark itself generated"; fi
  if grep -qF "hooks/$n.md" "$f"; then ok
  else bad "rule-source-$n" "fnd-$n.mdc does not name hooks/$n.md as its source"; fi
done
# store-access is the deliberate exception: the Cursor shim injects that text only where store files
# are detected, and a rule file cannot look at the workspace
if [ -f "$PLUGIN_DIR/rules/fnd-store-access.mdc" ]; then
  bad rule-store-access "fnd-store-access.mdc exists — the shim injects that text workspace-gated"
else ok; fi
# the host-specific rewrites are applied on the way out, not merely copied: the hook's own sentence
# tells a Claude Code session to unset a switch no other host reads
LEAN_RULE="$PLUGIN_DIR/rules/fnd-lean-code.mdc"
if grep -qF 'here, disable the rule instead' "$LEAN_RULE"; then ok
...
for v in cloud local; do
  ...
  # the same GEN_NOTE lead every other generated file carries — this one is a copy-out template,
  # so an in-place edit is lost on the next run and the note has to say so first
  if "$NODE_BIN" -e '
    const m = JSON.parse(require("fs").readFileSync(process.argv[1], "utf8"));
    process.exit(String(m._comment || "").startsWith("GENERATED by scripts/gen-host-adapters.cjs") ? 0 : 1);
  ' "$f" 2>/dev/null; then ok
  else bad "profile-gennote-$v" "$v profile _comment does not open with the GEN_NOTE lead"; fi
done
...
COPY="$TMP/fnd"
mkdir -p "$COPY"
cp -R "$PLUGIN_DIR/agents" "$PLUGIN_DIR/skills" "$PLUGIN_DIR/scripts" "$PLUGIN_DIR/.claude-plugin" \
  "$PLUGIN_DIR/hooks" "$PLUGIN_DIR/rules" "$COPY/" 2>/dev/null
...
# ------------------------------------------------------ scratch copy: the Cursor rules are owned --
# The rules sit among vendored and hand-authored .mdc files, where editing one in place looks like
# the obvious thing to do — so drift has to be caught the same way an adapter's is.
printf '\nhand-edited\n' >> "$COPY/rules/fnd-lean-code.mdc"
if "$NODE_BIN" "$CGEN" --check >"$TMP/r1" 2>&1; then bad drift-rule "--check passed an edited rule"
else grep -q 'rules/fnd-lean-code.mdc differs' "$TMP/r1" && ok \
  || bad drift-rule-msg "--check named no edited rule: $(head -1 "$TMP/r1")"; fi
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
else bad drift-rule-repair "a regeneration did not restore the rule byte-for-byte"; fi

# the hook is the one home: a sentence added there reaches the Cursor rule from that single edit
printf 'Probe line for the rules generator.\n' >> "$COPY/hooks/comment-discipline.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if grep -qF 'Probe line for the rules generator.' "$COPY/rules/fnd-comment-discipline.mdc"; then ok
else bad rule-propagation "a hook edit did not reach its Cursor rule"; fi
cp "$PLUGIN_DIR/hooks/comment-discipline.md" "$COPY/hooks/comment-discipline.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
else bad rule-propagation-restore "the hook probe left the scratch tree changed"; fi

# a hook edit landing ON a rewritten sentence must stop the generator instead: the Claude Code
# wording would tell a Cursor session to unset a switch that host never reads
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  fs.writeFileSync(p, fs.readFileSync(p, "utf8").replace("disable with `FND_LEAN=0`.", "disable it."));
' "$COPY/hooks/lean-code.md"
"$NODE_BIN" "$CGEN" >"$TMP/r2" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'fnd-lean-code.mdc' "$TMP/r2"; then ok
else bad rule-sub-unmatched "a hook edit under a rule rewrite generated anyway (exit $rc)"; fi
cp "$PLUGIN_DIR/hooks/lean-code.md" "$COPY/hooks/lean-code.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
else bad rule-sub-restore "the rewrite probe left the scratch tree changed"; fi

# the owned set is pinned in both directions: a hook added without a rules row would never reach
# Cursor, and a row dropped would leave its .mdc frozen on disk with every rail still green
printf '## Probe convention\n\nProbe body.\n' > "$COPY/hooks/probe-convention.md"
"$NODE_BIN" "$CGEN" --check >"$TMP/r3" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'hooks/probe-convention.md has no CURSOR_RULES row' "$TMP/r3"; then ok
else bad rule-own-hook "a hook with no rules row passed --check (exit $rc): $(head -1 "$TMP/r3")"; fi
rm -f "$COPY/hooks/probe-convention.md"
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  const src = fs.readFileSync(p, "utf8");
  const out = src.replace(/  \x27untrusted-content\x27: \{[\s\S]*?\n  \},\n/, "");
  if (out === src) { process.stderr.write("row not found\n"); process.exit(1); }
  fs.writeFileSync(p, out);
' "$CGEN"
"$NODE_BIN" "$CGEN" --check >"$TMP/r4" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'hooks/untrusted-content.md has no CURSOR_RULES row' "$TMP/r4"; then ok
else bad rule-own-row "a dropped rules row passed --check (exit $rc): $(head -1 "$TMP/r4")"; fi
cp "$PLUGIN_DIR/$GEN_REL" "$CGEN"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
else bad rule-own-restore "the ownership probes left the scratch tree changed"; fi

# an exemption is only honest while the shim injects that hook: a name added to the exempt list
# without shim wiring would reach no Cursor session at all, with every rail green
printf '## Probe convention\n\nProbe body.\n' > "$COPY/hooks/probe-convention.md"
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  const src = fs.readFileSync(p, "utf8");
...
"$NODE_BIN" "$CGEN" --check >"$TMP/r5" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'cursor-shim.cjs never injects' "$TMP/r5"; then ok
else bad rule-exempt-shim "an exempt hook the shim never injects passed --check (exit $rc): $(head -1 "$TMP/r5")"; fi
rm -f "$COPY/hooks/probe-convention.md"
cp "$PLUGIN_DIR/$GEN_REL" "$CGEN"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
...
"$NODE_BIN" -e '
  ...
  fs.writeFileSync(p, fs.readFileSync(p, "utf8")
    .replace("tools: Read, Grep, Glob, Bash", "tools: Read, Grep, Glob, Bash, Write"));
' "$COPY/agents/theme-explorer.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
caps_ok=1
fm "$COPY/agents-cursor/theme-explorer.md" | grep -q '^readonly:' && caps_ok=0
[ -n "$(tval "$COPY/agents-codex/theme-explorer.toml" sandbox_mode)" ] && caps_ok=0
fm "$COPY/agents-opencode/theme-explorer.md" | grep -q '^permission:' && caps_ok=0
if [ "$caps_ok" = 1 ]; then ok
else bad caps-readonly "a write tool in the canonical allowlist did not unsandbox the adapters"; fi
...
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  fs.writeFileSync(p, fs.readFileSync(p, "utf8").replace(/^disallowedTools: .*\n/m, ""));
' "$COPY/agents/jira-reader.md"
"$NODE_BIN" "$CGEN" >"$TMP/c1" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'fences no tools' "$TMP/c1"; then ok
else bad caps-unfenced "an agent fencing no tools generated anyway (exit $rc)"; fi
"$NODE_BIN" -e '
  const fs = require("fs"), src = process.argv[1], dst = process.argv[2];
  fs.writeFileSync(dst, fs.readFileSync(src, "utf8").replace(/^disallowedTools: /m, "tools: Read, Grep\ndisallowedTools: "));
' "$PLUGIN_DIR/agents/jira-reader.md" "$COPY/agents/jira-reader.md"
"$NODE_BIN" "$CGEN" >"$TMP/c2" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'carries both' "$TMP/c2"; then ok
else bad caps-both-fences "an agent mixing an allowlist into the denylist generated anyway (exit $rc)"; fi
cp "$PLUGIN_DIR/agents/jira-reader.md" "$COPY/agents/jira-reader.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
else bad caps-fence-restore "the fence probe left the scratch tree changed"; fi

# a fence wrapped onto a second line is still one YAML scalar: the tokens on the continuation
# line count, so a write tool there unsandboxes the adapters just as it would on the first line
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  const src = fs.readFileSync(p, "utf8");
  const out = src.replace("tools: Read, Grep, Glob, Bash", "tools: Read, Grep, Glob, Bash,\n  Write");
  if (out === src) { process.stderr.write("fence not found\n"); process.exit(1); }
  fs.writeFileSync(p, out);
' "$COPY/agents/theme-explorer.md" || bad caps-wrapped "the theme-explorer fence line moved"
"$NODE_BIN" "$CGEN" >"$TMP/c6" 2>&1
rc=$?
caps_ok=1
[ "$rc" = 0 ] || caps_ok=0
fm "$COPY/agents-cursor/theme-explorer.md" | grep -q '^readonly:' && caps_ok=0
...
fm "$COPY/agents-opencode/theme-explorer.md" | grep -q '^permission:' && caps_ok=0
if [ "$caps_ok" = 1 ]; then ok
else bad caps-wrapped "a write tool on a wrapped fence line was dropped (exit $rc): $(head -1 "$TMP/c6")"; fi
cp "$PLUGIN_DIR/agents/theme-explorer.md" "$COPY/agents/theme-explorer.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
else bad caps-wrapped-restore "the wrapped-fence probe left the scratch tree changed"; fi

# a fence written as a YAML flow list parses into tokens like `Write]`, which no write-tool name
# matches — the agent would derive read-only and ship sandboxed on three hosts with exit 0
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  fs.writeFileSync(p, fs.readFileSync(p, "utf8")
    .replace("tools: Read, Grep, Glob, Bash", "tools: [Read, Grep, Glob, Bash, Write]"));
' "$COPY/agents/theme-explorer.md"
"$NODE_BIN" "$CGEN" >"$TMP/c3" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q 'holds "\[Read"' "$TMP/c3"; then ok
else bad caps-flow-list "a YAML flow-list fence generated anyway (exit $rc): $(head -1 "$TMP/c3")"; fi
# a block list reaches the parser as an empty scalar: the message has to name that, not "no fence"
cp "$PLUGIN_DIR/agents/theme-explorer.md" "$COPY/agents/theme-explorer.md"
"$NODE_BIN" -e '
  const fs = require("fs"), p = process.argv[1];
  fs.writeFileSync(p, fs.readFileSync(p, "utf8")
    .replace("tools: Read, Grep, Glob, Bash", "tools:\n  - Read\n  - Write"));
' "$COPY/agents/theme-explorer.md"
"$NODE_BIN" "$CGEN" >"$TMP/c4" 2>&1
rc=$?
if [ "$rc" != 0 ] && grep -q '`tools:` is empty' "$TMP/c4"; then ok
else bad caps-block-list "a YAML block-list fence was not named as such (exit $rc): $(head -1 "$TMP/c4")"; fi
cp "$PLUGIN_DIR/agents/theme-explorer.md" "$COPY/agents/theme-explorer.md"
"$NODE_BIN" "$CGEN" >/dev/null 2>&1
if [ "$(fingerprint "$COPY")" = "$after" ]; then ok
